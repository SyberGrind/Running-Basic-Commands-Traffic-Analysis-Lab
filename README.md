...
----
# Running Powershell Commands & Traffic Analysis Lab.
...
| Category | Details |
|----------|---------|
| **Project** | Running Windows PowerShell & Traffic Analysis Lab |
| **Platform** | Microsoft Azure |
| **Cloud Environment** | Azure Resource Group & Virtual Network (VNet) |
| **Windows Machine** | Windows Virtual Machine |
| **Linux Machine** | Ubuntu Server 24.04 LTS |
| **Remote Access** | Remote Desktop Protocol (RDP), Secure Shell (SSH) |
| **Network Analysis Tool** | Wireshark |
| **PowerShell Commands Practiced** | `ipconfig`, `ipconfig /all`, `ping`, `hostname`, `tracert`, `nslookup`, `netstat`, `netstat -ano`, `ssh` |
| **Protocols Analyzed** | ICMP, SSH, TCP |
| **Security Configuration** | Azure Network Security Group (NSG) firewall rules for ICMP traffic |
| **Key Tasks Completed** | Azure infrastructure deployment, Windows & Linux VM provisioning, Remote Desktop connectivity, Windows PowerShell administration, Network diagnostics, DNS verification, Route tracing, Active connection analysis, Wireshark packet capture, ICMP traffic analysis, SSH traffic analysis, Azure Network Security Group (NSG) firewall testing, Secure remote Linux administration, Cloud resource cleanup |
| **Screenshots** | 74 |
...
🏗️🏢 Architecture
                           ...
                           Microsoft Azure
                    Resource Group: Network-Activities
                                   │
                    Azure Virtual Network (VNet)
                                   │
                 ┌─────────────────┴─────────────────┐
                 │                                   │
          WindowsVM                          Linux-UbuntuVM
      Windows Server/Windows             Ubuntu Server 24.04 LTS
                 │                                   │
       Remote Desktop (RDP)               SSH Remote Access
                 │                                   ▲
                 │                                   │
     ...
     Azure Network Security Group
      ICMP Rule Configured (Allow/Deny)
                 │
                 ▼
      Validate Firewall Behavior
      through PowerShell + Wireshark
>
<br>
...
In this phase, I deployed the foundational cloud infrastructure required for the lab within Microsoft Azure. I created a resource group, provisioned Windows and Linux virtual machines, configured networking, and established secure Remote Desktop access to prepare the environment for systems administration and network traffic analysis.
...
This final phase concludes the project by securely terminating the active SSH session and decommissioning all provisioned Azure virtual machines to ensure efficient resource management and environmental cleanup.
...
This lab reflects common responsibilities performed by IT Support Technicians, Help Desk Analysts, and Junior System Administrators. The ability to troubleshoot network connectivity using PowerShell, analyze packets with Wireshark, verify DNS resolution, inspect active connections, and test firewall rules are practical skills used daily when diagnosing network issues and supporting enterprise environments.
